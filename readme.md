...
### BeanPostProcessor
 - являются бинами
 - нужны для дополнительной инициализации бинов, обработки анотаций. Т.к. к BPP попадают уже бины со всеми своими зависимостями, а не BeanDefinitions
 - postProcessBeforeInitialization вызывается ДО initialization callbacks
 - postProcessAfterInitialization вызывается сразу ПОСЛЕ initialization callbacks
...
### Aware
 ...
 - нужны для инжекта специфичных бинов в bean lifecycle (например Environment или даже сам Context)
